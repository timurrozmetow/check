...
- **Серверные тесты** (Vitest — юнит + интеграция с БД). Нужна тестовая база `directorhub_test`:

  ```sql
  CREATE DATABASE directorhub_test CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
  ```
  ...
  Покрывают: изоляцию ролей (сотрудник не читает чужую задачу), уникальность email, полный поток решения директора, напоминания о дедлайнах, рендер графиков отчёта, форматирование.

- **E2E-тесты** (Playwright). Требуют запущенное приложение (`npm run dev`) и применённый seed:

  ```bash
  npm run test:e2e -w client
  ```

  Локально используют установленный Edge; в CI — bundled Chromium.

- **Быстрая проверка API** (сервер должен быть запущен):

  ```bash
  node scripts/smoke-test.mjs
  ```

CI (`.github/workflows/ci.yml`) на каждый push/PR прогоняет типизацию, серверные тесты (с MySQL-сервисом), сборку клиента и e2e.
